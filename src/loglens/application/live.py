...
class LiveDetector:
    def __init__(
        ...
        *,
        ...
        self.config = config or RunConfig(mode=mode, sensitivity=sensitivity, threshold=threshold)
    ...
    def feed_entry(self, entry: LogEntry) -> list[Anomaly]:
        ...
        if sev <= 2:
            ...
            a = _to_anomaly(entry, score, [f"{lvl} severity — surfaced immediately"], seq)
            self._mark(seq, a)
            out.append(a)
    ...
    def _due(self) -> bool:
        if len(self._win) < self.min_window:
            return False
        return (
            self._since >= self.rescore_every
    ...
    def _rescore(self, force: bool = False) -> list[Anomaly]:
        ...
        try:
            ...
            logger.debug("rescore failed (%s: %s)", type(exc).__name__, exc)
            return []
        new: list[Anomaly] = []
        for i in range(len(entries)):
            ...
            if self.suppress_info and self._benign_info(entries[i]):
                ...
                continue
            a = _to_anomaly(entries[i], det.scores[i], det.reasons[i], seqs[i])
            self._mark(seqs[i], a)
            new.append(a)
    ...
    def incident(self) -> bool:
        if not self._total:
            return False
        severe = sum(c for lv, c in self._levels.items() if get_severity(lv) <= 3)
        return severe / self._total >= 0.30

    def summary(self) -> dict[str, object]:
        return {
            "entries": self._total,
            "anomalies": self._hits,
    ...
    def ask(self, question: str, *, source_name: str = "live watch", **kw):
        ...
        return ask_about_anomalies(question, self.anomalies, source_name=source_name, **kw)

    def save_html(self, path: str, *, rca=None, source_name: str = "live watch") -> str:

        html = html_for_anomalies(
            self.anomalies, total_lines=self._total, source_name=source_name, rca=rca
        )
